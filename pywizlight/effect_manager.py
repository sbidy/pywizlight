...
class EffectDetails:
    ...
    gradient: bool = True
    init_step: int = (
        0  # Which step will play on the first LED (software head). For lightbulbs, set to 0 by default.
    )
    rand: int = 0
...
class PreviewEffect:
    ...
    def _validate_steps(self):
        ...
        if len(self.steps) > MAX_EFFECT_STEPS:
            raise ValueError(
                f"Effect cannot have more than {MAX_EFFECT_STEPS} steps, got {len(self.steps)}"
            )

        if self.details.init_step >= len(self.steps):
            raise ValueError(
                f"init_step ({self.details.init_step}) must be less than number of steps ({len(self.steps)})"
            )
    ...
    @classmethod
    def rainbow_fade(
        cls, duration: int = 10, step_duration: int = 1000
    ) -> "PreviewEffect":
    ...
    def breathing_white(cls, duration: int = 10) -> "PreviewEffect":
        ...
        steps = [
            EffectStep.from_kelvin(
                3000, 1000, 10, transition=500
            ),  # Dim warm white - slow fade in
            EffectStep.from_kelvin(
                3000, 1000, 100, transition=500
            ),  # Bright warm white - slow fade out
        ]
        details = EffectDetails(duration=duration)
        return cls(details, steps)
